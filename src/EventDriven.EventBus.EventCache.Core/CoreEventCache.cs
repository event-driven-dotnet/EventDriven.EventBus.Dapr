...
namespace EventDriven.EventBus.EventCache.Core;

/// <inheritdoc />
public class CoreEventCache : IEventCache
{
    ...
    public CoreEventCache(
            IOptions<EventCacheOptions> eventCacheOptions,
            ...
            CancellationToken cancellationToken = default)
    {
        ...
        if (!_eventCacheOptions.EnableEventCacheCleanup) return;
    ...
    private bool HasBeenHandledImpl(EventWrapper<IntegrationEvent> wrapper, string handlerTypeName)
    {
        ...
        var hasHandler = wrapper.Value?.Handlers.TryGetValue(handlerTypeName, out handlerInfo);
        var state = HandlerState.NotStarted;
        if (hasHandler is true && handlerInfo != null)
            state = handlerInfo.HanderState;
        var hasError =
            wrapper.Value != null &&
